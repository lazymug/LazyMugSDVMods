...
public class GameLocationPatcher
{
    ...
    {
        if (!__result || !who.hasBuff(BuffHelper.GetBuffIceTombId()))
            return;

        for (int index = __instance.characters.Count - 1; index >= 0; --index)
        {
            if (index < __instance.characters.Count && __instance.characters[index] is Monster monster && monster.IsMonster && monster.Health > 0 && monster.TakesDamageFromHitbox(areaOfEffect))
            {
    ...
    private static void FreezeEnemy(GameLocation location, Farmer farmer, Monster monster)
    {
        ...
        p.light.Value = true;
        p.debuffIntensity.Value = BuffConstants.IceTombFreezeDurationMs;
